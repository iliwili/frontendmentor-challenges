    ...
    <main>
      <section>
        <header className="flex justify-space-between items-center">
          ...
          <button
            className="header__button bg-transparent flex justify-center items-center"
            onClick={changeTheme}
          >
        ...
        <div
          className={`input-group bg-transition flex flex-row justify-space-between items-center ${
        ...
        >
          ...
          ></img>
          <input
            className="bg-transition bg-transparent"
          ...
          <button className="btn btn--right" onClick={searchUsername}>
            Search
          </button>
        </div>

        <article className="gh__info bg-transition">
          <header className="gh_info__header flex">
            <div className="gh_info__header--image border-50">
              {isLoaded && user != null ? (
                <img
                  src={user.avatar_url}
                  alt="profile-img"
            ...
            <div className="user-info flex flex-col justify-center content-center">
              <div className="w-60">
              ...
              <span className="user-info__joined">
                {isLoaded && user != null ? (
                  "Joined " + Moment(user.created_at).format("D MMM YYYY")
                ) : (
                  <Skeleton width="70" right={true} />
                )}
              </span>
            </div>
          </header>

          <p className="gh__info__summary">This profile has no bio.</p>

          <ul className="gh_info__stats bg-transition flex flex-row justify-space-between">
            <li>
              <div className="stat">
                <span className="stat--title">Repo</span>
                <h2>
                  {isLoaded && user !== null ? user["public_repos"] : "0"}
                </h2>
              </div>
            </li>
            <li>
              <div className="stat">
                ...
                <h2>{isLoaded && user !== null ? user["followers"] : "0"}</h2>
              </div>
            </li>
            <li>
              <div className="stat">
                <span className="stat--title">Following</span>
                <h2>{isLoaded && user !== null ? user["following"] : "0"}</h2>
              </div>
            </li>
          </ul>

          <ul className="gh_info__extra-info">
            <li
              className={`${
                user != null ? (user.location == null ? "empty" : "") : ""
              }`}
            >
            ...
            <li
              className={`${
                user != null ? (user.company == null ? "empty" : "") : ""
              }`}
            >
              <svg height="20" width="20" xmlns="http://www.w3.org/2000/svg">
                <g>
              ...
              {isLoaded && user !== null ? (
                user.company !== null ? (
                  <a href={`https://github.com/${user.company}`}>
                    {user.company}
                  </a>
                ) : (
                  <p>No Company</p>
